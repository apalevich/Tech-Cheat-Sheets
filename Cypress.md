...
```
> Как и человек, Cypress может кликнуть только на кнопку, находящуются в поле видимости (но это можно обойти с помощью параметра `force`)
...
> Cypress вводит текст в поля приближенно к тому, как это делает человек — посимвольно и с меняющейся скоростью
