...
Google рекомендует делать UI минималистичным, чтобы не отвлекать пользователя от веб-страницы. По это причине мы не будем касаться стилизации UI, хотя это и возможно с помощью CSS.
## Клик по иконке — Action

Иконка расширения — это первое, что видит пользователь. 

Extensions's action — это действие, которое происходит при клике на иконку. Оно регулируется Action API, доступным в Манифесте.

Есть два способа программирования UI на этом этапе: попап или действие
### Попап
...
Вместо попапа можно выполненить скрипт через хендлер Action API: `chrome.action.onClicked`
